...
#dirKPIS="VectorStore/EjemplosKPIS"
#ruta_ejemplosKPIS=r"DocumentosRequeridos/ExamplesKPI.txt"
dirKPIS="/app/VectorStore/EjemplosKPIS"
ruta_ejemplosKPIS=r"/app/DocumentosRequeridos/ExamplesKPI.txt"
...
def EjemplosDinamicosKPIS(query):
    from langchain_core.example_selectors import SemanticSimilarityExampleSelector
    from langchain_openai import OpenAIEmbeddings
    from langchain_core.prompts import ChatPromptTemplate,FewShotChatMessagePromptTemplate
    from langchain_chroma import Chroma
    ...
    if  os.path.exists(dirKPIS):
        #print("entro a la  carpeta de ejemplos")
        vector_store = Chroma(
        collection_name="SQLFinancieroKPIS",
        embedding_function=embeddings,
        persist_directory=dirKPIS,  # Donde guardar los datos localmente, elimínalo si no es necesario
        )
        results = vector_store.similarity_search(query, k=6)  # Cambia 'k' según el número de resultados que quieras obtener
        # Inicializa una variable para almacenar todos los resultados concatenados
        resultados = []
    ...
    #vectorstore.delete_collection()
        print("no existe")
        Examples=leerAEjemplos(ruta_ejemplosKPIS,2)#/app/DocumentosRequeridos/Examples.txt",2)
        vectorstore =Chroma()#
        example_selector = SemanticSimilarityExampleSelector.from_examples(
            Examples,
            OpenAIEmbeddings(),
...
def GetEjemploPromptKPIS(query):
    return EjemplosDinamicosKPIS(query)
##########################
