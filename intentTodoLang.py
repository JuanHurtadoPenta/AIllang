...
def ObtenerDB():
    ...
    db = SQLDatabase.from_uri(pg_uri, view_support = True, include_tables=['eeff_saldos_ebi_v'],sample_rows_in_table_info=3)           # Reduce la cantidad de datos cargados)
...
def NL2SQL(input,id_usuario,canal,id_padre):
    ...
    print(select_table.invoke({"question": input}))
...
Aquí está la información relevante de la tabla:
    ...
    generate_query=create_sql_agent(llm, db=db, agent_type="openai-tools", verbose=True,prompt= promptGSQL)
   
    # Crear un flujo que añade `table_names_to_use` antes de llamar al agente
    chain = (
        generate_query
    )
    #QuerySQL= chain.invoke({"question": input,"input": input})
    ...
    # Definir una función que obtiene la consulta y la pasa a db.run()
    def execute_query_with_db(query_data):
        print(query_data)
        query_data=query_data['query']
        query_data=query_data['output']
        query_data=extract_sql(query_data)
        query = filtrar_fecha(query_data)  # Obtener la consulta desde el diccionario
        if query == "La fecha que quieres consultar no tenemos en valores reales porque es mayor o igual a la fecha de hoy. Si quieres en presupuesto, vuelve a hacer tu consulta incluyendo que buscas el presupuesto.":
            return query
        else:
            return db.run(query)  # Ejecutar la consulta con db.run()

    # Crear la cadena con la asignación correcta
    ...
    # Usar la función intermedia en lugar de itemgetter con lambda
    chainA = (
        RunnablePassthrough.assign(query=chain)
        .assign(result=execute_query_with_db)  # Usar la nueva función
        | answer
    )
    QuerySQL=chainA.invoke({"question": input,"input": input })
    print(QuerySQL)
    QueryResponse=QuerySQL
    #print(QuerySQL)
    try:
...
consulta = "Cual es  balance general de  inmot para febrero 2024  el valor acumulado "
